...
| [`games/steam-headless/`](games/steam-headless/) | Steam + Sunshine on the Radeon 680M node, streamed to Moonlight (iOS, TV, laptop). LAN/tailnet only. | [games/steam-headless/README](games/steam-headless/README.md) |
...
| [`games/ror2/`](games/ror2/) | Experimental unofficial Risk of Rain 2 server. Windows game files come from the NAS over the existing read-only SMB claim; Proton and the companion plugin run in one pod. UDP 7777 is reachable at `rain.zachd.duckdns.org` from the tailnet. | [games/ror2/README](games/ror2/README.md) |
...
| [`life/early-internet-archive/`](life/early-internet-archive/) | Private, provenance-first browser for recovered forum posts, reviews, conversations, and images from 2005–2016. Source captures live on a NAS-backed RWX claim; two rootless readers copy the immutable SQLite search projection locally and stream media from the read-only claim. Tailnet-only DuckDNS ingress with Authelia forward-auth. | [life/early-internet-archive/README](life/early-internet-archive/README.md) |
...
| [`infra/node-exporter/`](infra/node-exporter/) | Host CPU/memory/disk/NIC metrics for every node. Values-only, DaemonSet, `hostNetwork` — which is not optional, since `/proc/net` resolves inside the reading process's netns and a cluster-network pod would report its own veth. The **only** source of visibility for Minecraft `:25565` and Jellyfin `:30096`, neither of which traverses Traefik. Collectors are opt-in to hold the series budget. | [infra/node-exporter/README](infra/node-exporter/README.md) |
...
- **Availability conventions.** Written down once here because they repeat in
  ...
  *Spread is `DoNotSchedule`, not a preference.* A `preferred` podAntiAffinity
  ...
  prediction rather than a property — and it fails silently. Spreading 2–3
  replicas across this many nodes remains satisfiable. Always include
