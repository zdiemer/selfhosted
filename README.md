...
| [`infra/alloy/`](infra/alloy/) | Grafana Alloy DaemonSet shipping the cluster's **external ingress** logs (Traefik access log, cloudflared, Authelia, plus pods labelled from their own `ingress.enabled`) and Traefik's `:9100` metrics to Grafana Cloud's free tier. Values-only against the upstream chart; no PVC, so nothing is pinned to a node. Before it there was no request-level visibility at all. | [infra/alloy/README](infra/alloy/README.md) |
