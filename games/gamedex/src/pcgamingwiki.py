...
_PAGE_DELAY = 1.0         # between pages, unconditionally
_RETRIES = 5
_BACKOFF = 8.0            # seconds, doubling: 8, 16, 32, 64, 128
...
class PcGamingWiki:
    def __init__(self, cache_dir: str = "/data/pcgamingwiki", ttl_days: int = 14):
        ...
        self._offset = 0                      # where a truncated pass got to
    ...
    def _load(self) -> None:
        try:
            ...
            if blob.get("version") == 1:
                ...
                self._offset = blob.get("offset") or 0
                ...
                if self._complete and fresh:
                    ...
                    return
                if self._map:
                    log.info("pcgamingwiki: resuming from offset %d (%d appids so far)",
                             self._offset, len(self._map))
    ...
    def _refresh_until_done(self) -> None:
        ...
        while not self._complete:
            ...
            log.info("pcgamingwiki: partial (%d appids, offset %d) — retrying in %.0fs",
                     len(self._map), self._offset, wait)
            time.sleep(wait)

    def _query(self, offset: int) -> list[dict]:
        """One page, with backoff. A 429 is a "slow down", not a "give up"."""
        wait = _BACKOFF
        for attempt in range(_RETRIES):
            r = self._s.get(API, timeout=60, params={
                "action": "cargoquery", "format": "json",
                "tables": _TABLES, "join_on": _JOIN, "fields": _FIELDS,
                "limit": _PAGE_SIZE, "offset": offset,
            })
            if r.status_code == 429:
                # Honour Retry-After when they bother to send one.
                retry_after = r.headers.get("Retry-After")
                delay = float(retry_after) if (retry_after or "").isdigit() else wait
                log.info("pcgamingwiki: 429 at offset %d — waiting %.0fs (attempt %d/%d)",
                         offset, delay, attempt + 1, _RETRIES)
                time.sleep(delay)
                wait *= 2
                continue
            r.raise_for_status()
        ...
        raise RuntimeError(f"rate-limited at offset {offset} after {_RETRIES} attempts")
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            log.warning("pcgamingwiki: refresh stopped at offset %d (%s)", offset, exc)
        if not games:
        ...
        log.info("pcgamingwiki: %d Steam appids indexed, offset %d (%s)", len(games), offset,
