...
| [`runner-cutover-vxp-zodemu.md`](runner-cutover-vxp-zodemu.md) | Finishing the move of vxp and zodemu onto self-hosted runners: adding `vxp` to the fine-grained PAT, creating the `arc-vxp` scale set, and reprovisioning the win11 guest so it runs one agent per repo with MSVC / rustup / .NET installed. All three repos' workflows are already pointed at it. | Code shipped 2026-09-11 — blocked on the GitHub PAT UI and an RDP session |
| [`carson.md`](carson.md) | "Mr. Carson" — personal CRM / assistant: todo tracker, birthday/anniversary reminders with Claude-researched gift links, calendar assistant (reads iCloud + Google, publishes its own ICS feed), and an email/iMessage watcher (IMAP + plug-in-triggered `pymobiledevice3` USB backups) with Ollama doing bulk extraction and gateway-scheduled Claude runs doing the judgment work. | Phase 1 built (`life/carson`), not deployed; iMessage path proven 2026-08-25 |
