...
because the constraints that shaped it — svclb answering on all ten node IPs,
DuckDNS in `tailnet` mode, iSCSI throughput — are cluster facts, not networking
trivia.

## Why

Every device in the house is on one flat `192.168.4.0/22`: ten k3s nodes, the
...
- **Isolating nodes from each other.** Flannel VXLAN and etcd want one L2
  segment with low latency. All ten nodes stay together.
...
| Nodes | `.26 .27 .28 .29 .30 .31 .32 .33 .37 .39` — all wired, single NIC, all negotiating 1000BASE-T |
| NAS | `192.168.4.36` (TrueNAS SCALE) — NFS, iSCSI, SMB |
| Handset | `192.168.4.24`, wifi |
| Router | eero Max 7 |
| Switch | unmanaged |

The switch is unmanaged by observation, not assumption: `zachd-ubuntu` and
`zachd-ubuntu-laptop-3` both receive the *same* LLDP frame, same chassis, same
port —
...
| Switch | 802.1q managed, ~14+ ports | Ten nodes, the NAS trunk, the handset, the eero uplinks. UniFi Standard 16 (~$179), or a pair of TL-SG108E at ~$25 each. |
...
the cluster has ten node addresses, NFS exports pinned by IP, svclb bound to
every node address, and etcd.

| VLAN | Subnet | Members |
|---|---|---|
| 1 (existing, untagged) | `192.168.4.0/22` | ten k3s nodes + NAS |
...
all ten nodes and `192.168.4.36` — NFS exports, the iSCSI portal, SMB, the
...
Pin it to one stable node IP — svclb answers on all ten, and the app holds a
single URL.
...
All ten nodes, the NAS, and the personal machines are one flat tailnet with
