...
because the constraints that shaped it — svclb answering on all nine node IPs,
DuckDNS in `tailnet` mode, iSCSI throughput — are cluster facts, not networking
trivia.

## Why

Every device in the house is on one flat `192.168.4.0/22`: nine k3s nodes, the
...
- **Isolating nodes from each other.** Flannel VXLAN and etcd want one L2
  segment with low latency. All nine nodes stay together.
...
| Nodes | `.26 .28 .29 .30 .31 .32 .33 .37 .39` — all wired, single NIC, all negotiating 1000BASE-T |
...
| Switch | 802.1q managed, ~14+ ports | Nine nodes, the NAS trunk, the handset, the eero uplinks. UniFi Standard 16 (~$179), or a pair of TL-SG108E at ~$25 each. |
...
the cluster has nine node addresses, NFS exports pinned by IP, svclb bound to
every node address, and etcd.

| VLAN | Subnet | Members |
|---|---|---|
| 1 (existing, untagged) | `192.168.4.0/22` | nine k3s nodes + NAS |
...
all nine nodes and `192.168.4.36` — NFS exports, the iSCSI portal, SMB, the
...
Pin it to one stable node IP — svclb answers on all nine, and the app holds a
single URL.
...
All nine nodes, the NAS, and the personal machines are one flat tailnet with
