#!/usr/bin/env bash
# Build the apartment-watch image and push it to GHCR.
#
...
#
# Re-run after editing the Dockerfile or anything under src/ or data/, then run
# upgrade.sh. Editing criteria.yaml needs NO rebuild — it's a ConfigMap.
#
...
echo "==> Done. Run upgrade.sh to point the CronJob at the new image."
echo "    (First push only: set the ghcr.io/zdiemer/apartment-watch package"
