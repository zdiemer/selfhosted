...
# local to, so on a DaemonSet all seven pods would probe all sixteen hostnames.
...
# Seven nodes rolled one at a time, each waiting on a readiness probe: the old
...
#
# It used to read `.items[0]` and that was fine when all seven pods shipped all
