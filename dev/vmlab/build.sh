#!/usr/bin/env bash
# Build and push the vmlab image. Run this before upgrade.sh whenever anything
...
# imagePullPolicy: IfNotPresent so that vmlab can still start on a node that
...
if [[ "${VMLAB_ALLOW_TAG_OVERWRITE:-0}" != "1" ]]; then
  if docker manifest inspect "${IMAGE}" >/dev/null 2>&1; then
    ...
    echo "      (VMLAB_ALLOW_TAG_OVERWRITE=1 to override — see the note above.)"
    exit 1
  fi
fi
